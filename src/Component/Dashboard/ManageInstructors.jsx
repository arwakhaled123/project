  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('https://localhost:7217/api/admin/all-users', {
          headers: {
        ...
        const allUsers = response.data.data.activatedUsers;
        
        const instructorsOnly = allUsers.filter(user => user.role === 'Instructor');
        const regularUsersOnly = allUsers.filter(user => user.role !== 'Instructor');

        const transformedInstructors = instructorsOnly.map(instructor => ({
        ...
        const transformedRegularUsers = regularUsersOnly.map(user => ({
          userId: user.userId,
          name: user.fullName,
        ...
        setRegularUsers(transformedRegularUsers);
        setLoading(false);
      } catch (err) {
...
const handleAddInstructor = async (formData) => {
  try {
    ...
    const response = await axios.post(
      `https://localhost:7217/api/admin/make-instructor/${formData.userId}`,
      {}, // body فارغ كما في المواصفات
      {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    if (response.data.success) {
      // تحديث القوائم مباشرة دون إنشاء مستخدم جديد
      const promotedUser = regularUsers.find(user => user.userId === formData.userId);
      ...
      if (promotedUser) {
        ...
        setRegularUsers(regularUsers.filter(user => user.userId !== formData.userId));
      }
  ...
  if (loading) return <div className="loading-message">Loading instructors...</div>;
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="dashboard-content">
        ...
        <div className="title-container">
          <h1 className='mng-inst'>Manage Instructors</h1>
        </div>
        <div className="action-buttons">
          <button className="add-instructor-btn" onClick={openPopup}>
            Add Instructor
          </button>
        </div>
        ...
        <div className="instructors-table">
          <div className="table-header">
            <div className="header-col id-col">ID</div>
            <div className="header-col name-col">Name</div>
            <div className="header-col email-col">Email</div>
          ...
          {filteredInstructors.length > 0 ? (
            filteredInstructors.map(instructor => (
              <div className="instructor-row" key={instructor.id}>
                ...
                <div className="instructor-col name-col">{instructor.name}</div>
                <div className="instructor-col email-col">{instructor.email}</div>
                <div className="instructor-col status-col">
