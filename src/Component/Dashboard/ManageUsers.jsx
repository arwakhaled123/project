...
const ManageUsers = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // تصفية المستخدمين للحصول على المستخدمين العاديين فقط (ليسوا مدربين)
        const regularUsers = response.data.data.activatedUsers.filter(user => user.role !== 'Instructor');
        ...
        const transformedUsers = regularUsers.map(user => ({
          ...
          registeredCourses: 0, // يمكنك الحصول على هذا من نقطة نهاية أخرى إذا لزم الأمر
  ...
  const handleSaveUser = async (updatedUserData) => {
    try {
      const token = localStorage.getItem('token');
      // You might want to call an update endpoint here
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://localhost:7217/api/admin/delete-user/${userId}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
